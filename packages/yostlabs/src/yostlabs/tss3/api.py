...
#closely match the actual source.
    ...
    def getHeading(self) -> ThreespaceCmdResult[float]: ...
    ...
    def getInterruptStatus(self) -> ThreespaceCmdResult[int]: ...
    ...
    def getStreamingBatch(self) -> ThreespaceCmdResult[list]:
        """Deprecated: Use generateStreamingPacket instead."""
        warnings.warn("getStreamingBatch is deprecated, use generateStreamingPacket instead.", DeprecationWarning, stacklevel=2)
        return self.generateStreamingPacket()
    def generateStreamingPacket(self) -> ThreespaceCmdResult[list]: ...
    ...
    def setTimestamp(self, timestamp: int) -> ThreespaceCmdResult[None]: ...
    ...
    def wmmGetField(self) -> ThreespaceCmdResult[list[float]]: ...
    def wmmUpdatePosition(self) -> ThreespaceCmdResult[None]: ...
    def wmmUpdateDate(self) -> ThreespaceCmdResult[None]: ...
    ...
    def setGpsData(self, latitude: float, longitude: float, altitude: float) -> ThreespaceCmdResult[None]: ...
...
#---------------------------------------SETTING PROTOTYPES----------------------------------------------
    ...
    def writeHeadingVector(self, value: list[float]) -> int:
        return self.write_settings(heading_vector=value)[0]
    
    def readHeadingVector(self) -> list[float]:
        return self.read_settings("heading_vector")["heading_vector"]
    
    def writeHeadingVectorAxis(self, value: str) -> int:
        return self.write_settings(heading_vector_axis=value)[0]

    def writeHeadingOffset(self, offset: float, mode: int) -> int:
        return self.write_settings(heading_offset=[offset, mode])[0]
    
    def readHeadingOffset(self) -> list[float, int]:
        return self.read_settings("heading_offset")["heading_offset"]
    ...
    def writeRtcSource(self, value: int) -> int:
        return self.write_settings(rtc_source=value)[0]
    
    def readRtcSource(self) -> int:
        return self.read_settings("rtc_source")["rtc_source"]
    
    def writeUtcOffset(self, value: float) -> int:
        return self.write_settings(utc_offset=value)[0]

    def readUtcOffset(self) -> float:
        return self.read_settings("utc_offset")["utc_offset"]
